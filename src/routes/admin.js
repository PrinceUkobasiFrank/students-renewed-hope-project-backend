...
const ADMIN_ROLES = ['super_admin', 'national_admin', 'state_coordinator', 'content_staff'];
...
router.get('/dashboard', requireAdminAuth(), asyncHandler(async (req, res) => {
  ...
  const totals = await pool.query(
    `SELECT
       ...
       (SELECT COUNT(*) FROM cards ${isCoordinator ? 'WHERE state_id = $1' : ''}) AS total_cards,
...
router.get('/students', requireAdminAuth(['super_admin', 'national_admin', 'state_coordinator']), asyncHandler(async (req, res) => {
  ...
  if (search) {
    conditions.push(`(st.first_name ILIKE $${i} OR st.last_name ILIKE $${i} OR st.email ILIKE $${i} OR st.vin ILIKE $${i} OR st.institution_name_freetext ILIKE $${i})`);
    params.push(`%${search}%`);
    i++;
  }
  if (level) {
  ...
  const dataResult = await pool.query(
    `SELECT st.id, st.first_name, st.last_name, st.email, st.phone, st.vin, st.level, st.created_at,
...
// POST /api/admin/admins — super_admin only. This is how you add state
// coordinators (or any other admin role) once you're logged in yourself —
// no separate signup page exists on purpose (see backend README).
router.post('/admins', requireAdminAuth(['super_admin']), asyncHandler(async (req, res) => {
  ...
  if (role === 'state_coordinator' && !state_id) {
    return res.status(400).json({ error: 'state_id is required for a state_coordinator.' });
  }
  ...
  const { rows } = await pool.query(
    `INSERT INTO admins (name, email, password_hash, role, state_id)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id, name, email, role, state_id`,
